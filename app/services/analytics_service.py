...
def get_dashboard_stats(db: Session) -> DashboardStats:
    ...
    Returns:
        DashboardStats object with aggregated data
    """
    ...
    total_active_tenants = db.query(func.count(Tenant.id)).filter(
        Tenant.status == TenantStatus.ACTIVE
    ).scalar() or 0

    # Rent collected this month
    rent_collected = db.query(func.sum(Payment.amount)).filter(
        Payment.status == PaymentStatus.PAID,
        func.extract('month', Payment.paid_date) == current_month,
        func.extract('year', Payment.paid_date) == current_year
    ).scalar() or 0.0

    # Rent outstanding (overdue payments)
    rent_outstanding = db.query(func.sum(Payment.amount)).filter(
    ...
    delayed_payments = db.query(func.count(Payment.id)).filter(
        Payment.status == PaymentStatus.OVERDUE
    ).scalar() or 0
